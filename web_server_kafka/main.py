...
async def upload_event(
    ...
    object_id: Optional[str] = Form(default=None),
...
async def upload_event_json(
    req: UploadJsonRequest,
    _key: str = Depends(verify_api_key),
    db: AsyncSession = Depends(get_db),
):
    existing = await db.execute(select(Event).where(Event.event_id == req.event_id))
    if existing.scalar_one_or_none():
        return JSONResponse({"status": "duplicate", "event_id": req.event_id})
    ...
    ws_images: dict = {}
    for url_field, img_type in [
        (req.plate_image_url, "plate"),
        (req.vehicle_image_url, "vehicle"),
        (req.frame_image_url, "frame"),
    ]:
        if url_field:
            ei = EventImage(
                event_id=req.event_id,
            ...
            db.add(ei)
            ws_images[f"{img_type}_image_url"] = url_field
    ...
    return JSONResponse({"status": "created", "event_id": req.event_id})
